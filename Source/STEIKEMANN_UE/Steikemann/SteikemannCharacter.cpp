...
void ASteikemannCharacter::GH_SetGrappleType(IGameplayTagAssetInterface* ITag, IGrappleTargetInterface* IGrapple)
{
	...
	if (IGrapple->IsStuck_Pure())
	{
		if (bPressedCancelButton) {
			PRINTLONG(2.f, "Pull target off wall");
...
void ASteikemannCharacter::PullDynamicTargetOffWall()
{
	...
	TimerManager.SetTimer(openmovement, [this]() { m_EMovementInputState = EMovementInput::Open; }, GH_PostPullingTargetFreeTime, false);
}
...
void ASteikemannCharacter::GH_SetInitialGrappleSmackAimingVector(FVector GrappledActorLocation)
{
	const FVector controldirection2D = GetControlRotation().Vector().GetSafeNormal2D();
	const FVector ToGrappledActor2D = FVector(GrappledActorLocation - GetActorLocation()).GetSafeNormal2D();
	const FVector right = FVector::CrossProduct(FVector::UpVector, ToGrappledActor2D);
	float Dot = FVector::DotProduct(controldirection2D, ToGrappledActor2D);
	float yaw = 1.f - FMath::Max(Dot, 0.2f);
	if (FVector::DotProduct(right, controldirection2D) < 0.f)
		yaw *= -1.f;
	
	GH_GrappleSmackAimingVector = FVector2D(GH_GrappleSmackAimingVector_Default.X, yaw);
}
...
bool ASteikemannCharacter::GH_ShowGrappleSmackImpactIndicator(FVector start, FVector end, float DrawTime)
{
	...
	FCollisionShape sphere = FCollisionShape::MakeSphere(125.f);
...
bool ASteikemannCharacter::PB_ValidTargetDistance(const FVector OtherActorLocation)
{
	...
	float PlayerZ = (GetActorLocation().Z - GetCapsuleComponent()->GetScaledCapsuleHalfHeight() * 0.7f);
	if (PlayerZ < OtherActorLocation.Z && Valid2D)
		return false;
...
bool ASteikemannCharacter::PB_Active_TargetDetection()
{
	...
	FHitResult Hit;
	FCollisionQueryParams Params("", false, this);
	if (GetWorld()->SweepSingleByChannel(Hit, GetActorLocation(), location, FQuat(), ECC_PogoCollision, capsule, Params)) {
		PB_Active_PogoTarget = Hit.GetActor();
		return true;
	}
	PB_Active_PogoTarget = nullptr;
	return false;
}

bool ASteikemannCharacter::Do_Pogo(const FHitResult& Hit)
{
	bool b{};
	if (m_EAirState == EAirState::AIR_Pogo) 
		return true;
...
bool ASteikemannCharacter::PB_Passive_IMPL(const FHitResult& Hit)
{
	...
	// Affect Pogo Target
	IAttackInterface* iattack = Cast<IAttackInterface>(Hit.GetActor());
	if (iattack) {
		iattack->IA_Receive_Pogo_Pure();
	}
...
bool ASteikemannCharacter::PB_Groundpound_Predeterminehit()
{
	...
	bool air = GetWorld()->SweepMultiByChannel(AirHits, GetActorLocation(), GetActorLocation() + FVector::DownVector, FQuat(), ECC_PogoCollision, Cap, Params);
	if (air) {
...
{
	if (!OverlappedComp->IsA(UCapsuleComponent::StaticClass()))
		return;
